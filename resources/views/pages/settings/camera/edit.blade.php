<form id="crudForm" action="{{ route('settings.cameras.update', $camera->id) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="row g-4">
            ...
            <select name="entity_id" class="form-select" required>
                @foreach ($entities as $entity)
                    <option value="{{ $entity->id }}"
                        {{ $entity->id == old('entity_id', $camera->entity_id) ? 'selected' : '' }}>
                        {{ $entity->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="name" value="{{ old('name', $camera->name) }}" class="form-control" required>
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $camera->ip_address) }}"
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="username" id="username" value="{{ old('username', $camera->username) }}"
                class="form-control">
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="snapshot_slug" id="snapshot_slug"
                value="{{ old('snapshot_slug', basename($camera->snapshot_url)) }}" class="form-control" required>

            <small class="text-muted d-block mt-1" id="snapshotPreview"></small>
                ...
                <div class="col-md-4">
                    <div class="form-switch switch-primary">
                        <input class="form-check-input camera-role" type="checkbox" name="is_primary" value="1"
                            {{ old('is_primary', $camera->is_primary) ? 'checked' : '' }}>
                ...
                <div class="col-md-4">
                    <div class="form-switch switch-info">
                        <input class="form-check-input camera-role" type="checkbox" name="is_secondary" value="1"
                            {{ old('is_secondary', $camera->is_secondary) ? 'checked' : '' }}>
                        <label class="form-check-label fw-medium">
                            Secondary Camera
                        </label>
                    </div>
