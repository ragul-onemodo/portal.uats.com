<form id="crudForm" action="{{ route('users.update', $user->id) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="row g-3">
            ...
            <select name="entity_id" class="form-select @error('entity_id') is-invalid @enderror" required>
                @foreach ($entities as $entity)
                    <option value="{{ $entity->id }}"
                        {{ old('entity_id', $user->entity_id) == $entity->id ? 'selected' : '' }}>
                        {{ $entity->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-12 col-md-6">
            <div class="switch-primary form-switch mt-4">
                ...
                <!-- Checkbox -->
                <input class="form-check-input" type="checkbox" name="status" id="status_edit" value="1"
